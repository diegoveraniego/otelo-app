...
BEGIN
    ...
    -- 4. Create chores
    FOR v_chore IN SELECT * FROM jsonb_to_recordset(p_chores) AS x(name TEXT, emoji TEXT, category TEXT, threshold_days INT, points INT) LOOP
        INSERT INTO public.chores (name, emoji, category, threshold_days, points, home_id)
        VALUES (v_chore.name, v_chore.emoji, v_chore.category, v_chore.threshold_days, COALESCE(v_chore.points, 1), v_home_id);
    END LOOP;
