-- Migration 025: Fix double-assignment bug and add explicit slot tracking

-- 1. Add metadata column to logs to explicitly pass slot info
ALTER TABLE public.logs ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;

-- 2. Update the sync trigger to prioritize explicit metadata slot selection
...
BEGIN
    ...
    -- Check if it's a pet feeding chore (excluding cats)
    IF (v_chore_name ILIKE '%dar comida%' OR v_chore_name ILIKE '%feed%' OR v_chore_name ILIKE '%alimentar%')
       AND v_chore_name NOT ILIKE '%gato%' THEN
        ...
        -- EXPLICIT SLOT via metadata (Fixes the race condition and gap guessing issues)
        IF NEW.metadata IS NOT NULL AND NEW.metadata->>'slot' IS NOT NULL THEN
            v_slot := NEW.metadata->>'slot';
        ELSE
            -- FALLBACK: Time-guessing for legacy logs or Dashboard completions
            IF v_hour < 12 THEN 
                v_slot := 'morning'; 
...
-- 3. Run a FULL clean historical rebuild (wiping fed_at and fed_by properly)
DO $$
DECLARE
    r RECORD;
    v_pet_id UUID;
    ...
    v_slot TEXT;
BEGIN
    -- PROPERLY CLEAR all feeding history to avoid double assignments
    -- Note: assigned_to and assigned_at are preserved!
    UPDATE public.feeding_slots SET feedings = NULL, fed_by = NULL, fed_at = NULL;

    FOR r IN 
        SELECT l.id, l.member_id, l.home_id, l.done_at, c.name AS chore_name, l.metadata
        FROM public.logs l
    ...
    LOOP
        ...
        IF v_pet_id IS NOT NULL THEN
            ...
            IF r.metadata IS NOT NULL AND r.metadata->>'slot' IS NOT NULL THEN
                v_slot := r.metadata->>'slot';
            ELSE
                IF v_hour < 12 THEN 
                    v_slot := 'morning'; 
                ELSIF v_hour >= 17 THEN
            ...
            VALUES (
                v_pet_id, r.home_id, v_week_start, v_day_of_week, v_slot, r.member_id, r.done_at,
