    ...
    <div className="animate-in fade-in duration-500 pb-8 space-y-6 mt-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#1E1E1E] dark:text-white" style={{ fontFamily: 'var(--font-bagnard)' }}>
              <span className="highlighter-yellow">Comida de {selectedPet?.name || 'Mascotas'}</span>
            </h2>
            ...
            {pets.map(pet => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all border ${selectedPetId === pet.id
                    ...
                    : 'bg-white text-[#1E1E1E]/60 dark:text-white/60 border-[#E5E6E6] dark:border-[#2C2C30] hover:border-[#3584E4]'
                  }`}
              >
                {pet.name}
              </button>
      ...
      <section className="space-y-3">
        ...
        {isLoading && slots.length === 0 ? (
          ...
          <div className="sketchy-border-alt p-4 transition-opacity duration-200">
            <FeedingWeekGrid
              slots={slots}
